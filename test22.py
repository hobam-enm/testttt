...
# ▶ 개선: 채팅 입력 처리 시 단 1회만 rerun 하도록 변경(깜빡임 최소화)
...
def ensure_state():
    defaults = {
        ...
        "__pending_user": None,  # ← 입력 대기열(리런 최소화용)
    }
...
if prompt := st.chat_input("예) 최근 24시간 태풍상사 반응 요약해줘 / 또는 영상 URL 붙여도 OK"):
    # 즉시 리런하지 않고 입력을 대기열에 저장
    st.session_state["__pending_user"] = prompt

# =========================
# 입력 처리: 같은 턴에서 처리 후 1회만 rerun
# =========================
if st.session_state.get("__pending_user"):
    user_query_raw = st.session_state.pop("__pending_user")

    # URL/자연어 분리
    url_ids = extract_video_ids_from_text(user_query_raw)
    natural_text = strip_urls(user_query_raw)
    has_urls = len(url_ids) > 0
    has_natural = len(natural_text) > 0

    # 사용자 메시지 기록
    st.session_state.chat.append({"role": "user", "content": user_query_raw})
    ...
    if not st.session_state.get("last_csv"):
        ...
        if has_urls and not has_natural:
            # (3) URL만 전달 → 해당 URL들의 댓글만 수집
            response = run_pipeline_first_turn(user_query_raw, extra_video_ids=url_ids, only_these_videos=True)
        elif has_urls and has_natural:
            # (2) 자연어 + URL → 기존 + URL 추가 수집
            response = run_pipeline_first_turn(user_query_raw, extra_video_ids=url_ids, only_these_videos=False)
        else:
            # (1) 자연어만 → 기존대로
            response = run_pipeline_first_turn(user_query_raw)
    else:
        # 후속 턴: 기존 규칙 유지 (추가 수집 없이 대화 문맥 기반 답변)
        response = run_followup_turn(user_query_raw)

    # 어시스턴트 메시지 기록
    st.session_state.chat.append({"role": "assistant", "content": response})

    # ✅ 입력창 정리 및 최신 상태 렌더를 위해 여기서만 1회 리런
    st.rerun()
